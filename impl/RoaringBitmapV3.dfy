  ...
  class RBitmap {
      ...
      ensures model == RBS.InsertRBitmapModel(x, old(model))
    {
      ...
      if isPresent {
        ...
        assert forall i :: 0 <= i < size && i != topChunk ==> ReprValid(i);
        // containers[topChunk].value.Insert(botChunk);
        // Repr := Repr + {containers[topChunk].value} + containers[topChunk].value.Repr;
        // model := RBS.InsertRBitmapModel(x, model);
        assert forall i, j :: 0 <= i < j < size  ==> topChunks[i] < topChunks[j];
        assert RBS.Valid(model);
        // This assertion does indeed slow the verifier
        // assert (forall i :: 0 <= i < size ==> model[i].1 == containers[topChunks[i]].value.Elements);
        // assert model == RBS.InsertRBitmapModel(x, old(model));
        assume Valid();
        // assert Valid();
        // assert model == RBS.InsertRBitmapModel(x, old(model));
      } else {
...
/*
    ...
    ensures bmapRes.Valid()
  {
    ...
    assume forall i :: 0 <= i < n ==>  0 <= newTopChunks[i] < U.Exp(2, 16) &&
                                       forall j :: i < j < n ==> newTopChunks[i] < newTopChunks[j];
    assume forall i :: 0 <= i < n ==>
...
*/
  ...
  class RoaringContainer {
    ...
    var arr: RoaringArrayBottom
    var bmap: RoaringBitmapBottom

    ghost predicate Valid()
      reads this, arr, arr.elems, bmap, bmap.bmaps
    {
      match containerType {
        case RoaringArray => 
          arr.Valid() &&
    ...
    {
      match containerType {
        case RoaringArray => present := arr.In(x);
        case RoaringBitmap => present := bmap.In(x);
      }
    }

    method Insert(x: nat16)
      requires Valid()
      modifies this, arr, bmap, arr.elems, bmap.bmaps
      ensures Valid()
      ...
      match containerType {
        case RoaringArray =>
          if arr.cardinality == 4096 {
            var bmapArray := C.ConvertArrayToBitmap(arr.elems);
            bmap := 
    ...
    ensures rbRes.Elements == CS.UnionContainers(rb1.Elements, rb2.Elements)
  {
    ...
    var c := 0;
    var i := 0;
    // assume RoaringBitmapElementsUpTo(tempBv64s, 0) ==
      ...
      //          RoaringBitmapElementsUpTo(rb1.bmaps, i) + RoaringBitmapElementsUpTo(rb2.bmaps, i)
    {
      ...
      i := i + 1;
      // assume RoaringBitmapElementsUpTo(tempBv64s, i) ==
      //       RoaringBitmapElementsUpTo(rb1.bmaps, i) + RoaringBitmapElementsUpTo(rb2.bmaps, i);
    }
    ...
    assume rbRes.Elements == CS.UnionContainers(rb1.Elements, rb2.Elements);
  }
    ...
    ensures newContainer.Elements == CS.UnionContainers(rb.Elements, arr.Elements)
  {
    ...
    assume newContainer.Elements == CS.UnionContainers(rb.Elements, arr.Elements);
  }
    ...
    ensures container.Elements == CS.UnionContainers(arr1.Elements, arr2.Elements)
  {
    ...
    assume container.Elements == CS.UnionContainers(arr1.Elements, arr2.Elements);
  }
  ...
  */
    ...
    ensures res.Elements == CS.UnionContainers(arr1.Elements, arr2.Elements)
  {
    ...
    assume res.Elements == CS.UnionContainers(arr1.Elements, arr2.Elements);
  }
  */
...
/*
  ...
  */
    ...
    ensures newContainer.Valid() && fresh(newContainer)
  {
    ...
    assume forall i, j :: 0 <= i < j < |seqElems| ==> seqElems[i] < seqElems[j];
